...
def load_images_and_labels(directory, labels, subset_ratio):
    X = []
    y = []
    ...
    for label in labels:
        ...
        subset_size = int(subset_ratio * total_files)
        print(f"Processing {subset_size} images in the {label} category...")
        
        subset_files = np.random.choice(files, size=subset_size, replace=False)
...
# Set the ratio for the subset (1/100th of the complete dataset)
subset_ratio = 0.01

# Load TRAINING images and labels
train_dir = os.path.join("..", root_dir, "train")
X_train, y_train = load_images_and_labels(train_dir, labels, subset_ratio)
...
X_valid, y_valid = load_images_and_labels(valid_dir, labels, subset_ratio)
...
X_test, y_test = load_images_and_labels(test_dir, labels, subset_ratio)
